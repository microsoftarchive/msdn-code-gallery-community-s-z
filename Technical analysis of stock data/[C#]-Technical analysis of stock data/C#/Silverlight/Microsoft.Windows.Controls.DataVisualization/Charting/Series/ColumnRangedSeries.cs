...
namespace Microsoft.Windows.Controls.DataVisualization.Charting
{
	...
	public sealed partial class ColumnRangedSeries : DataPointSingleSeriesWithAxes
	{
		/// <summary>
		...
		private void RedrawOtherColumnSeries(ISeriesHost seriesHost)
		{
			...
			foreach (ColumnRangedSeries series in seriesHost.Series.OfType<ColumnRangedSeries>().Where(series => series != this))
			{
		...
		*/
		...
		/// </summary>
		public IRangeAxis ActualIndependentRangeAxis 
		{ 
			get 
			{
				return this.InternalActualIndependentAxis as IRangeAxis; 
			} 
		}
		...
		{
			ColumnRangedSeries source = (ColumnRangedSeries)d;
