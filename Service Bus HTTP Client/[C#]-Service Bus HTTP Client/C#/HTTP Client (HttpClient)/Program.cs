...
namespace Microsoft.ServiceBus.ServicebusHttpClient
{
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Create HttpClientHelper and create SAS token or get token from ACS.
            HttpClientHelper httpClientHelper = new HttpClientHelper(ServiceBusNamespace, true, SasKeyName, SasKey); // Create SAS token.
            //HttpClientHelper httpClientHelper = new HttpClientHelper(ServiceBusNamespace, false, AcsIdentity, AcsKey); // Obtain ACS token.
            ...
            httpClientHelper.CreateEntity(topicAddress, topicDescription).Wait();

            // Query topic.
            Console.WriteLine("Query Topic ...");
            byte[] queryTopicResponse = httpClientHelper.GetEntity(topicAddress).Result;
            ...
            httpClientHelper.CreateEntity(subscriptionAddress, subscriptionDescription).Wait();

            // Query subscription.
            Console.WriteLine("Query Subscription ...");
            byte[] querySubscriptionResponse = httpClientHelper.GetEntity(subscriptionAddress).Result;
            ...
            sendMessage1.body = Encoding.UTF8.GetBytes("This is the first message.");
            ...
            httpClientHelper.SendMessage(topicAddress, sendMessage1).Wait();

            // Receive + delete message M1 from the subscription.
            Console.WriteLine("Receiving message 1 ...");
            ServiceBusHttpMessage receiveMessage1 = httpClientHelper.ReceiveAndDeleteMessage(subscriptionAddress).Result;
            ...
            httpClientHelper.SendMessage(topicAddress, sendMessage2).Wait();

            // Receive message M2.
            Console.WriteLine("Receiving message 2 ...");
            ServiceBusHttpMessage receiveMessage2 = httpClientHelper.ReceiveMessage(subscriptionAddress).Result;
            ...
            httpClientHelper.UnlockMessage(receiveMessage2.location).Wait();
            //httpClientHelper.UnlockMessage(subscriptionAddress, token, receiveMessage2.brokerProperties.MessageId, (Guid)receiveMessage2.brokerProperties.LockToken).Wait();
            //httpClientHelper.UnlockMessage(subscriptionAddress, token, (long)receiveMessage2.brokerProperties.SequenceNumber, (Guid)receiveMessage2.brokerProperties.LockToken).Wait();

            // Receive message M2 again.
            Console.WriteLine("Receiving message 2 again ...");
            receiveMessage2 = httpClientHelper.ReceiveMessage(subscriptionAddress).Result;
            ProcessMessage(receiveMessage2);
            ...
            //httpClientHelper.RenewLock(receiveMessage2.location).Wait();
            httpClientHelper.RenewLock(subscriptionAddress, receiveMessage2.brokerProperties.MessageId, (Guid)receiveMessage2.brokerProperties.LockToken).Wait();
            //httpClientHelper.RenewLock(subscriptionAddress, (long)receiveMessage2.brokerProperties.SequenceNumber, (Guid)receiveMessage2.brokerProperties.LockToken).Wait();
            ...
            //httpClientHelper.DeleteMessage(receiveMessage2.location).Wait();
            //httpClientHelper.DeleteMessage(subscriptionAddress, receiveMessage2.brokerProperties.MessageId, (Guid)receiveMessage2.brokerProperties.LockToken).Wait();
            httpClientHelper.DeleteMessage(subscriptionAddress, (long)receiveMessage2.brokerProperties.SequenceNumber, (Guid)receiveMessage2.brokerProperties.LockToken).Wait();
            ...
            httpClientHelper.SendMessageBatch(topicAddress, sendMessage3).Wait();

            // Receive and delete messages M3, M4 and M5.
            for (int i = 3; i <= 5; i++)
            {
                Console.WriteLine("Receiving message {0} ...", i);
                ServiceBusHttpMessage receiveMessage = httpClientHelper.ReceiveAndDeleteMessage(subscriptionAddress).Result;
            ...
            ServiceBusHttpMessage receiveMessage6 = httpClientHelper.ReceiveAndDeleteMessage(subscriptionAddress).Result;
            ...
            httpClientHelper.SendMessage(topicAddress, sendMessage7).Wait();
            ms7.Close();
            ...
            httpClientHelper.DeleteEntity(topicAddress).Wait();
        }
