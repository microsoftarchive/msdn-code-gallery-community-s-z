 ...
 _
            ...
            If ((viewName IsNot Nothing) AndAlso (viewName.Length > 0)) Then
                Using viewAdapter = New ViewsAdapter(_sharepointBaseUri, _credential)
                    ...
                    If (viewId.Count() = 1) Then
                        Return viewId(0)
                    Else
                        Throw New SharePointUtilityOnline.SharePointUnhandledException( _
                ...
                Select _
                 <result ID=<%= row.@ID.Split(",")(0) %>>
                     ...
                     <row><%= _
                              ...
                              Where fld.LookupFieldDisplay = SharePointUtilityOnline.DataObject.ColumnData.EncodedFieldDisplayEnum.DisplayRaw _
        ...
        Private Shared Function LookupErrorCode(ByVal errorCode As String) As String
            ...
            Select Case Int32.Parse(hexNumber, Globalization.NumberStyles.AllowHexSpecifier)
                ...
                Case SharePointUtilityOnline.SharePointResult.Success
                    Return "Success"
                Case SharePointUtilityOnline.SharePointResult.ChangeConflict
                    Return "The changes requested conflict with those made by another client."
                Case SharePointUtilityOnline.SharePointResult.InvalidValueForField
                    Return "A generic error has been encountered, such as an invalid value being specified for a Field."
                Case SharePointUtilityOnline.SharePointResult.InvalidFieldValue
                    Return "Invalid column value in the data being updated (ex: Date Time, Multiple Lookup value, Person, etc)."
                Case SharePointUtilityOnline.SharePointResult.InvalidDateTime
                    Return "Invalid Date/Time value in the data being updated."
                Case SharePointUtilityOnline.SharePointResult.InvalidNumber
                    Return "Invalid Number value in the data being updated."
                Case SharePointUtilityOnline.SharePointResult.InvalidCurrency
                    Return "Invalid Currency value in the data being updated."
                Case SharePointUtilityOnline.SharePointResult.InvalidHyperlink
                    Return "Invalid Hyperlink value in the data being updated."
                Case SharePointUtilityOnline.SharePointResult.InvalidBoolean
                    Return "Invalid boolean (0/1) value in the data being updated."
                Case SharePointUtilityOnline.SharePointResult.ItemDoesNotExist
                    Return "List item referred to in the request does not exist."
                Case SharePointUtilityOnline.SharePointResult.FolderAlreadyExists
                    Return "Folder already exists."
                Case SharePointUtilityOnline.SharePointResult.UnspecifiedError
