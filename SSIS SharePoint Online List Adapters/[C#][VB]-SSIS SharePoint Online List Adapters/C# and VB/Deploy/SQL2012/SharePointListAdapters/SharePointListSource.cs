...
{
    [DtsPipelineComponent(DisplayName = "SharePoint List Source Online",
        CurrentVersion = 8,
        IconResource = "Microsoft.Samples.SqlServer.SSIS.SharePointListAdaptersOnline.Icons.SharePointSource.ico",
    ...
    public class SharePointListSource : PipelineComponent
    {
        ...
        /// <returns></returns>
        private List<SharePointUtilityOnline.DataObject.ColumnData> GetAccessibleSharePointColumns(string sharepointUrl, string listName, string viewName)
        {
            ...
            if (expandLookups)
            {
                ...
                var lookupColumns =
                    ...
                    where c.LookupFieldDisplay != SharePointUtilityOnline.DataObject.ColumnData.EncodedFieldDisplayEnum.DisplayRaw
                    select c;

                var exclusionList =
                    from x in accessibleColumns
                    join c in lookupColumns on x.SourceFieldName equals c.SourceFieldName
                    where x.LookupFieldDisplay == SharePointUtilityOnline.DataObject.ColumnData.EncodedFieldDisplayEnum.DisplayRaw
                    select x;
        ...
        public override void PrimeOutput(int outputs, int[] outputIDs, PipelineBuffer[] buffers)
        {
            ...
            timer.Start();
            var listData = SharePointUtilityOnline.ListServiceUtility.GetListItemData(
